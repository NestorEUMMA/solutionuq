<?php

namespace app\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\Tramoipsfa;

/**
 * TramoipsfaSearch represents the model behind the search form of `app\models\Tramoipsfa`.
 */
class TramoipsfaSearch extends Tramoipsfa
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['IdTramoIpsfa'], 'integer'],
            [['TramoIpsfa', 'TechoIpsfa', 'TechoIpsfaSig'], 'number'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Tramoipsfa::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'IdTramoIpsfa' => $this->IdTramoIpsfa,
            'TramoIpsfa' => $this->TramoIpsfa,
            'TechoIpsfa' => $this->TechoIpsfa,
            'TechoIpsfaSig' => $this->TechoIpsfaSig,
        ]);

        return $dataProvider;
    }
}
